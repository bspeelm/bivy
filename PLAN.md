...
want an account. bivy never authenticates, reads no cookies and holds no
credential — so most of what a client like this normally has to get right does
...
extractor for a stream. Nothing that identifies the person: no account, no
cookie, no API key, no persistent identifier bivy invents. IP-level exposure is
...
jar, accepts no API key and stores no token. This section exists to be short
